...
- Document the live Obsidian smoke root safety guard and metadata-only fast no-op proof boundary.
...
- Refuse to recursively delete a custom live-smoke root unless it is marked as a smoke-test directory, and require Obsidian `ctime` alongside size/mtime before the restored-state fast no-op can fire.
- Keep rolled `Synology Sync Logs/history/` diagnostics out of Git-backed vault sync so generated logs cannot turn an otherwise unchanged sync into a new commit.
